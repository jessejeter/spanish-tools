...
def backfill_trans_context(service):
    ...
    if pending:
        print(f"  Writing final batch of {len(pending)} updates...")
        execute_with_retry(
            service.spreadsheets().values().batchUpdate(
                spreadsheetId=SPREADSHEET_ID,
                body={"valueInputOption": "RAW", "data": pending},
...
def make_analysis_prompt(spanish_word, english=None, sense=None):
    ...
    return (
        ...
        "Grammatical context: Flag only what is genuinely surprising or important to "
        "know — write 'Grammatical context: none.' if nothing is worth flagging, which "
        "is expected most of the time. If there is something to flag, state it in one "
        "short sentence. Do not mention things that are standard or expected — only "
        "note what would catch a learner off guard. Never list things that don't apply. "
        "Things worth flagging: unexpected or meaning-changing gender (e.g. el problema, "
        "el/la capital); irregular plural; invariable adjective; gustar-type structure; "
        "preposition(s) the word takes; notable conjugation irregularities (not spelling "
        "changes that merely preserve pronunciation); defective usage; subjunctive "
        "trigger; transitive vs. intransitive when the distinction matters; preterite vs. "
        "imperfect meaning shift; reflexive vs. non-reflexive meaning split when -se "
        "creates a meaningfully different sense; notably colloquial, formal, literary, or "
        "vulgar register; primarily Spain or Latin America usage.\n\n"
